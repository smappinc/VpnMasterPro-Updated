...
public class VpnProfileDataSource
{
	...
	String Best_server    ,  USER_NAME,PASSWORD,REMOTE_ID,strong_swan_alias,best_server,Server_ip;
...
//			String Best_server    ,  USER_NAME,PASSWORD,REMOTE_ID,strong_swan_alias,best_server;
//			SharedPreferences sharedPreferences = appContext.getSharedPreferences("", MODE_PRIVATE);
//			USER_NAME = sharedPreferences.getString("username_ipsec", "");
//			PASSWORD = sharedPreferences.getString("password_ipsec", "");
//			REMOTE_ID = sharedPreferences.getString("ipsec_server", "");
//			strong_swan_alias = sharedPreferences.getString("strong_swan_alias", "");
	...
	private VpnProfile getHardCodedVpnProfile(){
		...
		USER_NAME = sharedPreferences.getString("username_ipsec", "");
		PASSWORD = sharedPreferences.getString("password_ipsec", "");
		REMOTE_ID = sharedPreferences.getString("default_ip", "");
		strong_swan_alias = sharedPreferences.getString("ikev2_alias", "");
		Server_ip = sharedPreferences.getString("default_ip", "");

		Log.d("ALL_DATA" , ""+ USER_NAME +
				"  " + PASSWORD + "   " + REMOTE_ID + "         " + Server_ip + "  " + strong_swan_alias);

		VpnProfile profile = new VpnProfile();
		profile.setId(1);
		profile.setUUID(UUID.fromString("5565c7cf-3f4d-4b1d-8da1-1842963e2a4e"));
		profile.setName("Usa VPN");
		profile.setGateway(Server_ip);
		profile.setVpnType(VpnType.IKEV2_EAP);
		profile.setUsername(USER_NAME);
		profile.setPassword(PASSWORD);
		profile.setCertificateAlias(strong_swan_alias);
		profile.setUserCertificateAlias(null);
		profile.setMTU(null);
		profile.setPort(null);
		profile.setSplitTunneling(null);
		profile.setLocalId(null);
		profile.setRemoteId(REMOTE_ID);
		profile.setExcludedSubnets(null);
		profile.setSelectedAppsHandling(VpnProfile.SelectedAppsHandling.SELECTED_APPS_DISABLE);
		profile.setSelectedApps("");
		profile.setNATKeepAlive(null);
		profile.setFlags(0);
		profile.setIkeProposal(null);
		profile.setIkeProposal(null);
		profile.setDnsServers(null);
