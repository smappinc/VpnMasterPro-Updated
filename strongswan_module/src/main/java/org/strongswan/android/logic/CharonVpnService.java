...
//					profile.setDnsServers(null);
					...
					if (profile != null)
					{
						...
						SharedPreferences pref = PreferenceManager.getDefaultSharedPreferences(this);
						pref.edit().putString(Constants.PREF_MRU_VPN_PROFILE, profile.getUUID().toString()).apply();
					}
				}
			}
	...
	private Notification buildNotification(boolean publicVersion)
	{
		...
		else
		{
			...
			switch (state)
			{
				case CONNECTING:
					...
					add_action = true;
					//extra added by aws to remove notification
					removeNotification();
					break;
				case CONNECTED:
		...
		if (!publicVersion)
		{
			if (add_action)
			{
				...
				PendingIntent pending = PendingIntent.getActivity(getApplicationContext(), 0, intent,
																  PendingIntent.FLAG_UPDATE_CURRENT);
				builder.addAction(R.drawable.ic_notification_disconnect, getString(R.string.disconnect),
						pending);*/
			}
			if (error == ErrorState.NO_ERROR)
			{
