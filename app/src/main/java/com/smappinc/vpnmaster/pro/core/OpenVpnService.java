...
import com.smappinc.vpnmaster.pro.VpnProfile;
...
public class OpenVpnService extends VpnService {
  ...
  public static Boolean checkCounter=false;
	...
	public static int mConnectionState = OpenConnectManagementThread.STATE_DISCONNECTED;
...
public void getTimeV(){

	countDownTimer =new CountDownTimer(totalSeconds * 1000, intervalSeconds * 1000) {

		public void onTick(long millisUntilFinished) {
			Log.e("secondselapsed: " , String.valueOf((millisUntilFinished) / 1000));
			TimeDown= (millisUntilFinished) / 1000;
			if(getConnectionState()==OpenConnectManagementThread.STATE_DISCONNECTED){
			...
			//	Toast.makeText(getBaseContext(), "value "+ (millisUntilFinished) / 1000, Toast.LENGTH_SHORT).show();
			//	btnConnect.setProgress((int) (((totalSeconds * 1000 - millisUntilFinished) / 1000)*10));
...
*/
	...
	public static String formatElapsedT(long startTime) {
		/*StringBuilder sb = new StringBuilder();
		...
		sb.append(String.format("%1$02d:%2$02d", startTime / 60, startTime % 60));*/
		StringBuilder sb = new StringBuilder();
		OpenVpnService.TimeDown %= 60 * 60 * 24;
		sb.append(String.format("%1$02d:", OpenVpnService.TimeDown / (60 * 60)));
		OpenVpnService.TimeDown %= 60 * 60;

		sb.append(String.format("%1$02d:%2$02d", OpenVpnService.TimeDown / 60, OpenVpnService.TimeDown % 60));
	...
	public synchronized void setStats(LibOpenConnect.VPNStats stats) {
		if (stats != null) {
			mStats = stats;
		}
		wakeUpActivity();
